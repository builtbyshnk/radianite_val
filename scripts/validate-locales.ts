...
const registry = await readJson<LocaleRegistry>(
  path.join(root, "locale-registry.json"),
)
...
if (!(await exists(uiFallbackPath)))
  throw new Error(`Missing UI fallback catalog ${fallback}.json`)
if (!(await exists(rustFallbackPath)))
  throw new Error(`Missing Rust fallback catalog ${fallback}.json`)
...
for (const locale of registry.locales) {
  ...
  tags.add(locale.tag)
  if (!/^[a-z]{2,3}(?:-[A-Z][a-z]{3})?(?:-[A-Z]{2}|-\d{3})?$/.test(locale.tag))
    throw new Error(`Invalid BCP 47 tag ${locale.tag}`)
  if (
    !locale.nativeName ||
    !locale.englishName ||
    !["ltr", "rtl"].includes(locale.direction)
  )
    throw new Error(`Incomplete metadata for ${locale.tag}`)
  ...
  if (locale.ui && (await exists(uiFile))) {
    ...
    for (const [key, value] of translated) {
      if (!uiFallback.has(key))
        throw new Error(`${locale.tag} UI has unknown key ${key}`)
      if (value === "") continue
      ...
      if (expected !== actual)
        throw new Error(
          `${locale.tag} UI placeholder mismatch at ${key}: expected [${expected}], got [${actual}]`,
        )
    }
  }
  ...
  if ((locale.ui || locale.rpc) && (await exists(rustFile))) {
    ...
    for (const [key, value] of translated) {
      if (!rustFallback.has(key))
        throw new Error(`${locale.tag} Rust catalog has unknown key ${key}`)
      if (value === "") continue
      const expected = placeholders(rustFallback.get(key)!, /%{([\w.-]+)}/g)
      const actual = placeholders(value, /%{([\w.-]+)}/g)
      if (expected !== actual)
        throw new Error(
          `${locale.tag} Rust placeholder mismatch at ${key}: expected [${expected}], got [${actual}]`,
        )
    }
  }
}
...
for (const name of await readdir(uiDir)) {
  if (
    name.endsWith(".json") &&
    !name.startsWith("_") &&
    !tags.has(name.slice(0, -5))
  )
    throw new Error(`UI catalog ${name} is missing from locale-registry.json`)
}
for (const name of await readdir(rustDir)) {
  if (
    ...
    throw new Error(`Rust catalog ${name} is missing from locale-registry.json`)
}

console.log(
  `Validated ${registry.locales.length} locale(s); checked ${uiCatalogs} UI and ${rustCatalogs} Rust catalog(s); partial catalogs use ${fallback} fallback.`,
)
