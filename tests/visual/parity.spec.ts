...
async function capture(page: Page, name: string) {
  ...
  const expected = PNG.sync.read(await readFile(path.join(baselineDir, `${name}.png`)))
  ...
  expect(ratio, `${name} changed ${(ratio * 100).toFixed(3)}%`).toBeLessThanOrEqual(0.001)
}
...
for (const size of sizes) {
  test(`capture React parity states at ${size.width}x${size.height}`, async ({ page }) => {
    ...
    await expect(page.getByRole("dialog")).toBeVisible()
    await expect(page.getByText("Faster startup", { exact: true })).toBeVisible()
