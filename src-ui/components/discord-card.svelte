  ...
  {#snippet icon()}<IconBrandDiscord
    ...
    />{/snippet}{#snippet action()}<Button
      ...
      >{#if rpc.enabled}<IconCircleStop
        ...
        : locale.t("common.enable")}</Button
    >{/snippet}
  <div class="flex flex-col gap-3">
    <div class="flex items-center gap-2 text-sm">
      <span
        class:bg-success={rpc.connected}
        class:bg-muted-foreground={!rpc.connected}
        class="size-2 rounded-full"
      ></span><span
        class:text-success={rpc.connected}
        class:text-muted-foreground={!rpc.connected}
        class="font-medium"
        >{rpc.connected
          ? locale.t("common.connected")
          : locale.t("common.disconnected")}</span
      >
    </div>
    <p class="text-xs text-muted-foreground">
      {locale.t("discord.friendsSee")}
    </p>
    {#if preview}<div
        class="rounded-lg border border-white/5 bg-[#232428] p-3 text-[#dbdee1]"
        dir="auto"
      >
        <p
          class="mb-2 text-[0.65rem] font-bold tracking-wide text-[#b5bac1] uppercase"
        >
          {locale.t("discord.playing")}
        </p>
        <div class="flex gap-3">
          <div class="relative size-[52px] shrink-0">
            <div
              class="flex size-full items-center justify-center overflow-hidden rounded-lg bg-[#1a1b1e]"
            >
              {#if largeUrl}<img
                  src={largeUrl}
                  alt=""
                  class="size-full object-cover"
                />{:else}<AppIcon class="size-9 rounded-md" />{/if}
            </div>
            <span
              class="absolute -bottom-1 -end-1 flex size-5 items-center justify-center overflow-hidden rounded-full bg-[#232428] ring-2 ring-[#232428]"
              >{#if agentUrl}<img
                  src={agentUrl}
                  alt=""
          ...
          <div class="min-w-0 flex-1 text-sm leading-tight">
            <p class="truncate font-semibold text-white">{preview.name}</p>
            <p class="truncate text-[0.7rem] text-[#b5bac1]">
              {preview.details}
            </p>
        ...
        class="rounded-lg border bg-[#1a1b1e] p-3 text-sm text-muted-foreground"
      >
        {canToggle
          ? locale.t("discord.noMatch")
          : locale.t("discord.notConfigured")}
      </div>{/if}
  </div>
</Panel>
