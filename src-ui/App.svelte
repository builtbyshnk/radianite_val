<script lang="ts">
  ...
  let settingsOpen = $state(false),
    SettingsDialog = $state<any>(null)
  async function openSettings() {
    SettingsDialog = (await import("@/components/settings-dialog.svelte"))
      .default
    settingsOpen = true
  }
  onMount(() => {
    void r.initialize()
    if (!import.meta.env.PROD) return () => r.destroy()
    ...
    const blockKeys = (e: KeyboardEvent) => {
      ...
      if (
        ...
        (modifier && key === "r") ||
        (modifier && key === "u") ||
        (modifier && e.shiftKey && ["c", "i", "j"].includes(key))
      ) {
        e.preventDefault()
        e.stopPropagation()
      }
